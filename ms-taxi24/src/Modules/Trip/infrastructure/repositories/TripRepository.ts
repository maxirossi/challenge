...
import { toTripDTO } from '@Trip/model/Mappers/TripMapper';
...
export class TripRepository {
  ...
  async getTripById(uuid: string): Promise<GenericResponse<TripDTO>> {
    try {
  ...
  async delete(uuid: string): Promise<InternalResponse> {
    try {
      const trip = await this.prisma.trip.findUnique({ where: { uuid } });
